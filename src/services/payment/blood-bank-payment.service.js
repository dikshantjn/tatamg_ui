...
import { updateBloodBankPaymentCompleted } from '../User/BloodBank/blood-bank.service';

class BloodBankPaymentServiceClass {
  constructor() {
    this.razorpay = null;
  ...
  async processPayment({ amount, bookingId, user, description }, onSuccess, onFailure) {
    try {
      ...
      const options = {
        ...
        description: description || `Blood Bank Booking #${bookingId}`,
        handler: async (response) => {
          try {
            // Update backend payment status after successful payment
            await updateBloodBankPaymentCompleted(bookingId);
...
export const BloodBankPaymentService = new BloodBankPaymentServiceClass();
export default BloodBankPaymentService; 