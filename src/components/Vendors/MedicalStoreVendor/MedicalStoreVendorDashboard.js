...
const MedicalStoreVendorDashboard = () => {
  ...
  const menuItems = [
    { text: 'Dashboard', icon: <Dashboard />, path: '/vendor/pharmacy/dashboard' },
    { text: 'Inventory', icon: <Inventory />, path: '/vendor/pharmacy/inventory' },
    { text: 'Orders', icon: <ShoppingCart />, path: '/vendor/pharmacy/orders' },
    { text: 'Customers', icon: <People />, path: '/vendor/pharmacy/customers' },
    { text: 'Analytics', icon: <Assessment />, path: '/vendor/pharmacy/analytics' },
    { text: 'Settings', icon: <Settings />, path: '/vendor/pharmacy/settings' },
  ];

  // Sample dashboard data
  const dashboardStats = [
    { title: 'Total Products', value: '1,245', icon: <Inventory />, color: 'primary' },
    { title: 'Today Orders', value: '45', icon: <ShoppingCart />, color: 'secondary' },
    { title: 'Revenue (₹)', value: '67,230', icon: <TrendingUp />, color: 'success' },
    { title: 'Pending Deliveries', value: '12', icon: <LocalShipping />, color: 'warning' },
  ];

  if (loading) {
    return (
      <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh' }}>
        <Typography>Loading medical store dashboard...</Typography>
      </Box>
    );
  }
    ...
    <Box sx={{ display: 'flex', height: '100vh' }}>
      ...
      <Box component="main" sx={{ flexGrow: 1, p: 3, marginTop: '64px' }}>
        <Typography variant="h4" gutterBottom>
          Welcome back, {vendorData?.email?.split('@')[0] || 'Medical Store'}!
        </Typography>
                  ...
                  {[1, 2, 3].map((item) => (
                    <ListItem key={item} sx={{ px: 0 }}>
                      ...
                      <ListItemText
                        ...
                        secondary={`₹${1200 + item * 100} • 3 items`}
                      />
          ...
          <Grid item xs={12} md={6}>
            <Card>
              <CardContent>
                ...
                <List>
                  {['Paracetamol 500mg', 'Vitamin C Tablets', 'First Aid Kit'].map((product, index) => (
                    <ListItem key={index} sx={{ px: 0 }}>
                      ...
                      <ListItemText
                        primary={product}
                        secondary={`${50 + index * 10} units sold`}
                      />
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
