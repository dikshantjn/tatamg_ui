...
const LabTestVendorDashboardContent = () => {
  ...
  const menuItems = [
    { text: 'Dashboard', icon: <Dashboard />, path: '/vendor/lab-test/dashboard' },
    { text: 'Tests', icon: <Science />, path: '/vendor/lab-test/tests' },
    { text: 'Appointments', icon: <Schedule />, path: '/vendor/lab-test/appointments' },
    { text: 'Patients', icon: <People />, path: '/vendor/lab-test/patients' },
    { text: 'Reports', icon: <Assignment />, path: '/vendor/lab-test/reports' },
    { text: 'Analytics', icon: <Assessment />, path: '/vendor/lab-test/analytics' },
    { text: 'Settings', icon: <Settings />, path: '/vendor/lab-test/settings' },
  ];
  ...
  if (loading) {
    return (
      <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh' }}>
        <Typography>Loading lab test dashboard...</Typography>
      </Box>
    );
  }
    ...
    <Box sx={{ display: 'flex', height: '100vh' }}>
      ...
      <AppBar position="fixed" sx={{ zIndex: theme.zIndex.drawer + 1 }}>
        <Toolbar>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            >
              {vendorData?.email?.split('@')[0] || 'Lab Test'}
            </Button>
          </Box>
        </Toolbar>
      </AppBar>
          ...
          <Card sx={{ mb: 2 }}>
            <CardContent sx={{ textAlign: 'center' }}>
              ...
              <Typography variant="h6" gutterBottom>
                {vendorData?.generatedId || 'Lab Test'}
              </Typography>
              <Chip 
                label="Pathology/Diagnostic" 
                color="primary" 
                size="small" 
                variant="outlined"
              />
      ...
      <Box component="main" sx={{ flexGrow: 1, p: 3, marginTop: '64px' }}>
        <Typography variant="h4" gutterBottom>
          Welcome back, {vendorData?.email?.split('@')[0] || 'Lab Test'}!
        </Typography>
